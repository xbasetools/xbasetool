g:xcheck=0;
function checkmailer(mailerid){
	if(xcheck > 9){
		bootbox.alert("<b>Wait</b> - Other 10 checking operation is executed!");
	} else {
		xcheck++;
		document.getElementById("workornot"+mailerid).innerHTML = "<button class='btn btn-info btn-sm disabled'>Checking...</button>";
		document.getElementById("buy"+mailerid).innerHTML = '<button type="button" class="btn btn-danger btn-sm disabled">Buy</button>';
		var postdata = "id="+mailerid;
		$.ajax({
		type: "post",
		url: "mailer_ops.php?check",
		data: postdata,
		success: function(html){
		document.getElementById("workornot"+mailerid).innerHTML = html;
		document.getElementById("buy"+mailerid).innerHTML = '<button type="button" class="btn btn-danger btn-sm" onclick=buymailer("'+mailerid+'")>Buy</button>';
		xcheck--;
		}
		});
	}
}	
function buymailer(mailerid){
	bootbox.confirm({
			...
			if(result==true){
				...
				setTimeout(function() {
				 ...
				 if(html.match(/Add Balance/)){
					...
					document.getElementById("buy"+mailerid).innerHTML = html;
				 }
				}
				});	
			}else{
